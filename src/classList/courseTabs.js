export default function courseTabs() {
    ...
    // Helper: apply inline fallbacks to beat any remaining !important utilities
    ...
    const mo = new MutationObserver(muts => {
      muts.forEach(m => {
        m.addedNodes.forEach(n => {
          ...
          n.querySelectorAll?.('div.cursor-pointer.space-x-2.flex.justify-center.items-center')
            .forEach(tab => {
              if (tab.dataset.darkCourseTab === "1") return;
              tab.dataset.darkCourseTab = "1";
              inlineForce(tab);
