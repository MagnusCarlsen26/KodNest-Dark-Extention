...
export function darkifyComboboxDropdowns() {
    ...
    const ro = new MutationObserver(muts=>{
      for (const m of muts){
        ...
        if (m.target.matches('button[role="combobox"][aria-controls]')) {
          tagDropdownFromButton(m.target);
        }
      }
    });
    document.querySelectorAll('button[role="combobox"][aria-controls]').forEach(btn=>{
      ro.observe(btn, { attributes:true, attributeFilter:["aria-expanded"] });
    });
    ...
    const mo = new MutationObserver(muts=>{
      for (const m of muts){
        for (const n of m.addedNodes){
          ...
          const id = n.id;
          if (!id) continue;
          const btn = document.querySelector(`button[role="combobox"][aria-controls="${CSS.escape(id)}"]`);
          if (btn) tagDropdownFromButton(btn);
        }
      }
    });
    mo.observe(document.documentElement, { childList:true, subtree:true });
