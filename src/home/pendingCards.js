export default function pendingCards() {
    ...
    // 4) Initial pass: target .dashboard-card
    document.querySelectorAll('.dashboard-card').forEach(darkify);
  
    // 5) Watch for API/SPA insertions
    ...
    window.__darkRedCardsStop?.();
    window.__darkRedCardsStop = () => observer.disconnect();
  
    console.log("Darkify: red alert dashboard-cards are now dark with red accents.");
}